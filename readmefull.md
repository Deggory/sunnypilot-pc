...
```
# 🤖 AI ORANGE PI 5 OPENPILOT PORT ARCHITECTURE & MANIFEST
**DOCUMENT PURPOSE**: This file is a machine-readable AND human-readable full system graph. It contains every single modification required to port any modern Openpilot/Sunnypilot fork to the Orange Pi 5 (RK3588, Linux 24.04) running IMX415 via V4L2 WebCam, without Radar, with or without IMU, and bypassing standard Comma hardware dependencies.

If an AI (like VSCode Copilot) is reading this, treat this as your master instruction set. Apply these diffs and architectural rules directly to the target codebase.

## 📊 1. SYSTEM GRAPH & HARDWARE SPECS
*   **SBC**: Orange Pi 5 / 5B / 5 Plus
*   **SoC**: Rockchip RK3588 (CPU/GPU/NPU)
*   **NPU Backend**: `rknn-toolkit-lite2` (Crucial for `modeld` inference)
*   **OS**: Ubuntu 24.04 Linux (Runs Openpilot as a "PC" `PC=True`)
*   **Camera**: IMX415 (Mapped as a standard V4L2 webcam, pure vision)
*   **IMU**: Optional / LSM6DS3 via I2C (Pin 3 / 5)
*   **Display**: HDMI Out with USB Touchscreen (Native Qt5 HID Linux support)
*   **Storage Environment**: SD Card / NVMe. *Constraint: Limit I/O to prevent wear.*

---

## 📂 2. FOLDER-WISE MODIFICATION MANIFEST
Proceed down the codebase tree systematically. Apply these changes to adapt the fork.

### 📁 Root Configuration (`/`)
1. **`pyproject.toml`**
   *   **Goal:** Enable NPU acceleration.
   *   **Action:** Add `"rknn-toolkit-lite2"` to Python dependencies block.
2. **`launch_env.sh`**
   *   **Goal:** Set fallback parameters for Openpilot without Comma hardware.
   *   **Action:** Append the following overrides:
       ```bash
       export PC=1                  # Enforce PC mode
       export WEBCAM=1              # Fallback camera to V4L2
       export NO_IMU=1              # Bypass IMU crash (remove if LSM6DS3 is wired)
       export SCALE=2.5             # UI scaling for 1080p HDMI Touchscreen (Default UI is 800x420)
       export USE_WEBCAM=1
       ```

### 📁 System & Daemon Management (`/system/`)
3. **`system/manager/process_config.py`**
   *   **Goal:** Extreme SD Card wear prevention and CPU conservation.
   *   **Rationale:** Standard OP constantly writes video/logs to NVMe. SD cards will physically melt or wear out.
   *   **Action:** Set `enabled=False` for all logging daemons:
       *   `loggerd` -> `enabled=False`
       *   `logmessaged` -> `enabled=False`
       *   `logcatd` -> `enabled=False`
       *   `proclogd` -> `enabled=False`
       *   `deleter` -> `enabled=False`
       *   `encoderd` -> `enabled=False`

4. **`system/hardware/hw.h` & `hw.cc`**
   *   **Goal:** Return `PC` mode true.
   *   **Action:** Ensure `Hardware::PC()` consistently returns `true` on OP5 architecture.

### 📁 UI & Display Configuration (`/selfdrive/ui/`)
5. **`selfdrive/ui/qt/qt_window.h` & `qt_window.cc`**
   *   **Goal:** Adapt display size for external HDMI.
   *   **Rationale:** Sunnypilot natively dropped PC UI to `800x420` (commit `bf3f07e7`). To scale to 1080p, env var `SCALE` is checked.
   *   **Action:** No direct C++ alterations strictly needed if `export SCALE=xxx` is handled by Qt, but verify `DEVICE_SCREEN_SIZE = {800, 420};` exists and `util::getenv("SCALE", 1.0f);` is respected in `setMainWindow()`.

### 📁 Model & Path Planning (`/selfdrive/modeld/`)
6. **`selfdrive/modeld/`**
   *   **Goal:** RKNN initialization.
   *   **Action:** NPU model execution relies on `rknn-toolkit-lite2`. Ensure runner invokes `.rknn` models and not `.thneed` (which is SNPE/Qualcomm specific).

---

## 📡 3. LSM6DS3 I2C IMU INTEGRATION (RECOMMENDED)
By default, we used `NO_IMU=1`. To drastically improve steering smoothness and Laneless mode centering, wire an LSM6DS3 IMU into the Orange Pi 5.

### Hardware Wiring (OPi5 26-pin Header):
*   **VCC** -> Pin 1 (3.3v)
*   **GND** -> Pin 6
*   **SDA** -> Pin 3 (I2C-5)
*   **SCL** -> Pin 5 (I2C-5)
...
2. Add: `overlays=i2c5`.
3. Reboot and check: `sudo i2cdetect -y 5`. Expected address: `0x6A` or `0x6B`.

### Codebase Modifications:
1. **Remove the Fake Flag:** In `launch_env.sh`, **delete** `export NO_IMU=1`.
2. **Re-route the C++ Sensord:**
   *   **Target File:** `system/sensord/sensors/lsm6ds3_accel.cc` & `lsm6ds3_gyro.cc` (or `system/sensord/sensord.cc` architecture dependent).
   *   **Action:** Find the hardcoded I2C bus initialization (usually bus `0` for Comma 3X) and change it to bus `5`.
   *   *Example Diff:*
     ```cpp
     // Old
     I2CBus *bus = new I2CBus(0);
     // New
     I2CBus *bus = new I2CBus(5);
     ```
3. **Recompile:** Run `scons -u -j8 system/sensord/`.

---

## 🚀 4. BUILD & RUN WORKFLOW
1. **Dependencies:** Install through `uv` environment.
2. **Compile:** Run `scons -j8` in root. Memory IPC via `/dev/shm` ensures zero-disk-write compilation and webcam buffer streaming.
3. **Launch:** Run `./launch_openpilot.sh`.
4. **Touch Input:** Via standard Linux/Qt5 HID stack. No custom touchscreen code required.

---
**AI CHECKLIST FOR NEW FORKS:**
- [ ] Inject `rknn-toolkit-lite2` into `pyproject.toml`.
- [ ] Turn off `loggerd`, `encoderd`, `logmessaged`, `logcatd`, `deleter` in `process_config.py`.
- [ ] Add `export WEBCAM=1`, `export PC=1`, `export SCALE=x.x` to `launch_env.sh`.
- [ ] If NO IMU: inject `export NO_IMU=1`.
- [ ] If hardware IMU added: shift `sensord.cc` I2C bus to `5` and rebuild `sensord`.
- [ ] Ensure MADS / Vision only / Laneless UI is enabled in Qt setup.
---

## 📜 5. HISTORICAL RK3588/OPI5 COMMIT REFERENCE
The following are the core commits applied to the sunnypilot fork to adapt the architecture for the RK3588. When porting other forks, these structural changes inside `modeld` and the build system must be replicated:

*   **`f39cc16f7` feat(more): Adapter to rk3588** - Baseline RK3588 adapter structure.
*   **`4f1fe8be7` build: install rknn-toolkit-lite2 python dependencies for Orange Pi 5** - Adds RKNN package to build sequence.
*   **`c67fbf154` modeld: integrate RKNN HAL, harden inference safety, and add camera resilience** - Critical! RKNN relies on HAL (Hardware Abstraction Layer) for safe NPU memory release and camera fallback looping.
*   **`f1a252655` fix(modeld): enforce correct RK3588 NPU core partitioning (vision=0,1; policy=2)** - Splits the NPU load so the massive vision model doesn't block the policy outputs.
*   **`ff16cb777` fix(modeld): ensure big core pinning (4-7) for RK3588** - Forces the AI thread loops onto the `A76` big cores for minimum latency.
*   **`cb7b335ca` fix(rknn): Enable CPU thread pinning for Rockchip NPU on PC builds** - Ensures low-latency thread priority.

*When updating a fork: ensure `modeld.py` correctly pins vision to NPU Cores 0/1, policy to NPU Core 2, and the processes themselves are tied to CPU Cores 4-7.*

## 🔍 6. MASTER LIST OF 2025 SUNNYPILOT PORT MODIFICATIONS
For context to developers (or AIs), here are the absolute lowest-level fundamental code changes introduced into the core repos to "Make OP Run" on the RK3588 with no Comma hardware. If porting a fresh branch, reproduce these logic architectures.

### 📌 `locationd.py` (True NO_IMU Bypass)
**Commit:** `b51be4e29` `feat(selfdrive): Make op run`
Standard Openpilot will instantly crash `pose / rednose` if `sensorEvents` are not present. To fix this without an actual IMU:
*   We modified `selfdrive/locationd/locationd.py` to completely ignore `accelerometer` and `gyroscope` updates.
*   We intercept `carState` and inject `msg.aEgo` (forward acceleration) and `msg.yawRate` (steering angle CAN conversion) directly into `kf.predict_and_observe` masquerading as `ObservationKind.PHONE_ACCEL` and `PHONE_GYRO`. This generates perfect 3D odometry with no physical gyro hooked up!

### 📌 `ui/ui.h` & `camera.py` (IMX415 Camera Array)
**Commit:** `712bcb2be` `feat(camera): Change camera focal to imx415`
Comma 3X strictly expects AR0231 sensors. Our IMX415 sensors have differently shaped lenses.
*   **Focal Shift:** Inside `common/transformations/camera.py`, the `_ar_ox_config` focal length was rewritten from `1095.0` to `900.0` for 1280x720 resolutions.
*   **UI Perspective:** The C++ Qt rendering `FCAM_INTRINSIC_MATRIX` in `selfdrive/ui/ui.h` was fundamentally rewritten to reflect the new `900.0` focal array matrix, preventing the display from warping the road borders on our HDMI touchscreens.

### 📌 Timing Adjustments (`realtime.py` & `cereal/services.py`)
**Commit:** `f39cc16f7` `feat(more): Adapter to rk3588`
The RK3588 NPU + OP's complex IPC messaging stack cannot process inference at 100hz.
*   `realtime.py`: `DT_MDL` was shifted from `0.05` to `0.1` (10Hz).
*   `cereal/services.py`: Sub-system frequencies like `drivingModelData`, `modelV2`, `livePose`, and `radarState` were dropped by exactly half (10Hz to 5Hz, 20Hz to 10Hz) to prevent RKNN timeouts and keep system load below 30ms thresholds.

## 🎛️ 7. COMPLETE 12-MONTH HISTORICAL MODIFICATIONS (ALL FOLDERS)
Beyond the core architectural structural changes required to boot on the Orange Pi, the original author (`mx`) submitted 13 distinct commits over the last 12 months to perfect the driving characteristics. When duplicating this onto a vanilla fork, ensure these domain-specific mathematical adjustments are carried over:

### 📁 `selfdrive/controls/` & `selfdrive/car/` (Longitudinal Tuning)
*   **Commits:** `3ad69b20b`, `ddf8fa492`, `28b99cc23`
*   **Files Modified:** `selfdrive/controls/lib/longitudinal_mpc_lib/long_mpc.py`, `selfdrive/controls/lib/longitudinal_planner.py`, `selfdrive/car/cruise.py`
*   **What was changed:** The baseline Openpilot acceleration/braking curves can be quite abrupt for certain retrofit vehicles. The mathematical weight of `LEAD_DANGER_FACTOR` was shifted, and the Model Predictive Control (MPC) logic for longitudinal path planning was softened. This provides much smoother gas/brake responses specifically tuned for retrofitted standard cars.

### 📁 `selfdrive/locationd/` (The NO_IMU Toggle)
*   **Commit:** `306a880ca` `feat(locationd): Allow no imu`
*   **Files Modified:** `selfdrive/locationd/locationd.py`
*   **What was changed:** While the initial "Make op run" commit force-bypassed the IMU by injecting `msg.aEgo` and `msg.yawRate` into the Kalman Filter, this specific commit abstracted it gracefully by officially introducing the `export NO_IMU=1` environment variable. It dynamically toggles whether to trust the `sensorEvents` I2C sockets, or to fall back gracefully to purely Vision/CAN odometry.

### 📁 `tools/` (Debugging Tooling)
*   **Commits:** `6e91b9c2c`, `cab810f02`
*   **Files Modified:** `tools/plotjuggler/juggle.py`, `tools/car_porting/auto_fingerprint.py`, `tools/lib/route.py`
*   **What was changed:** Openpilot relies on Comma's API for logs. These commits modified out-of-car developer tooling (`cabana` / `plotjuggler`) to safely load legacy local `/storage/` router files bypassing the need for an active Comma Prime cloud subscription.

### 📁 `selfdrive/ui/` & `common/` (Display & Camera Refinements)
*   **Commits:** `95af03c09`, `bf3f07e7a`
*   **What was changed:** Final stabilization commits setting the IMX415 FOV and tuning the `QSize DEVICE_SCREEN_SIZE = {800, 420}`.

*(End of Master Porting Guide)*

---

### 🧠 Deep Dive: The "CAN-Bus Masquerade" (NO_IMU Hack)

#### The Problem: Why Openpilot Normally Crashes Without an IMU
In standard Openpilot, a daemon called `locationd` runs an advanced algorithm called a Kalman Filter (rednose).
Its job is to calculate the precise 3D orientation of the car in real-time (pitch, yaw, roll, and lateral sliding). To do this, the Kalman Filter mathematically demands two high-frequency arrays of data to function:

* **3-Axis Accelerometer Data** (X, Y, Z forces — including gravity).
* **3-Axis Gyroscope Data** (Pitch-rate, Roll-rate, Yaw-rate).

If `locationd` does not receive these specific arrays from the physical I2C sensors (`sensorEvents`), it refuses to initialize. The path planner panics, the steering gives up, and the UI throws a red error.

#### The Genius Hack: The "CAN-Bus Masquerade"
Instead of letting the system crash, the port's author rewrote the `handle_log` function inside `locationd.py`.

Instead of listening for physical IMU chips, the code intercepts `carState`—which is the data package coming directly from your car's internal computers via the Panda over the CAN bus.

**1. Faking the Accelerometer**
Your car already knows how fast it is accelerating (from its wheel speed sensors) and passes this over the CAN bus as `msg.aEgo`.

The code takes that single number and artificially builds a fake 3-axis accelerometer array:
```python
meas = np.array([msg.aEgo, 0, -9.81])
```
* **X-Axis (`msg.aEgo`)**: Plugs in the car's real forward acceleration.
* **Y-Axis (`0`)**: Tells the system the car is not sliding sideways (side-slip = 0).
* **Z-Axis (`-9.81`)**: This is the genius part. The Kalman filter constantly checks for Earth's gravity to know which way is "down." The code dynamically hardcodes `-9.81 m/s²` (the literal force of gravity) so the filter mathematically believes the Orange Pi is sitting perfectly flat and upright.

It then shoves this fake array directly into the Kalman Filter:
```python
self.kf.predict_and_observe(sensor_time, ObservationKind.PHONE_ACCEL, meas)
```
The filter processes it, perfectly happy, believing it just read a real hardware accelerometer.

**2. Faking the Gyroscope**
Your car also knows how sharply you are turning the steering wheel, and the CAN bus converts this physics data into `msg.yawRate` (the rotational speed of the car).

The code builds a fake 3-axis Gyroscope array:
```python
meas = np.array([0, 0, -msg.yawRate])
```
* **X-Axis / Pitch (`0`)**: Tells the system the car is not pointing up a hill or down a hill.
* **Y-Axis / Roll (`0`)**: Tells the system the road is perfectly flat (no banking).
* **Z-Axis / Yaw (`-msg.yawRate`)**: Plugs in the car's actual steering rotation.

Again, it pushes this fake array into the filter masquerading as `ObservationKind.PHONE_GYRO`.

#### The Final Result
By doing this, Openpilot is successfully tricked.

Because `locationd` is receiving perfectly formatted, mathematically valid X/Y/Z matrices that align with gravity and CAN-bus reality, it outputs a "Valid 3D Pose."
The Vision Model receives this valid pose, draws the driving path perfectly on your screen, and begins steering the car—completely unaware that there is no physical IMU chip attached to your Orange Pi.

**The only limitation:** Because pitch and roll are hardcoded to `0`, the system cannot instantly "feel" when you drive over a speed bump or transition onto a steep hill (it relies purely on the camera seeing the horizon shift instead). But for steering laterally down a highway, it is a flawless bypass!

#### How the `NO_IMU` Flag Controls This Hack
The codebase is designed with a brilliant built-in toggle via the `NO_IMU` environment variable (`locationd.py` Lines 116-128). 

*   **Scenario A: KEEP `export NO_IMU=1` (Pure Vision Mode):** 
    The system skips the gyro masquerade entirely (`if not NO_IMU:`). It ignores the car's steering angle (`msg.yawRate`) and relies 100% on **Visual Odometry** (calculating turns purely by watching the camera frames shift over time). *Use this if your car does not broadcast accurate wheel speed or steering data over the CAN bus.*
*   **Scenario B: REMOVE `export NO_IMU=1` (CAN-Bus Masquerade Mode):** 
    The code officially activates the hack described above. It grabs the vehicle's `msg.yawRate` and injects it into the system pretending to be a physical hardware gyroscope. *Use this if your car perfectly supports CAN-bus steering data, but you physically lack an IMU chip on your Orange Pi.*

This gives developers the ultimate software toggle: fallback to pure camera vision, or fallback to CAN-bus physics math!

---

### 🧠 Deep Dive: Neural Network Lateral Control (NNLC)
Normally, steering is controlled by simple mathematical formulas (PID or INDI loops). In this fork, steering utilizes **NNLC (Neural Network Lateral Control)**. Instead of hardcoded math, lateral (steering) control uses a miniature trained neural network dedicated purely to calculating perfect steering torque.

#### The 16-Input Matrix
The NN Torque Model accepts a mathematically flattened array of roughly 16 specific inputs, processing them 100 times per second:

**1. The Current State Variables:**
*   `CS.vEgo`: The car's exact current speed.
*   `_desired_lateral_accel`: The exact lateral G-force the Vision Model *wants* you to turn at (target).
*   `_measurement`: The exact lateral G-force the car is *actually* experiencing (reality).
*   `friction_input`: A calculation of lateral jerk to overcome the steering wheel's deadzone/stiffness.

**2. The Angle Math (Gravity Compensation):**
*   `roll` & `pitch`: Evaluates the physical tilt of the car. It calculates how much gravity is pulling forward versus sideways, and adjusts the steering torque so you don't oversteer on a downhill banked curve!

**3. The "Time Travel" Variables (Past & Future):**
*   **Past**: Looks at requested steering acceleration at **-0.3, -0.2, and -0.1 seconds ago** to understand the momentum of continuous turns.
*   **Future**: Looks ahead at the Vision Model's path at **+0.3, +0.6, +1.0, and +1.5 seconds**. If a sharp turn is coming, it starts applying micro-amounts of steering torque *now* to pre-load the steering rack perfectly.

#### Compatibility with `NO_IMU=1` & MADS
Because we safely run **MADS** (Modular Assistive Driving System), you can control gas/brakes dynamically while the AI handles lateral steering. Even with `export NO_IMU=1` active, NNLC performs flawlessly. 

Openpilot's "Visual Odometry" calculates synthetic `pitch`, `roll`, and `lateral acceleration` purely from camera frames. The NNLC swallows this visual math, combines it with the car's steering fingerprint, and generates ultra-smooth lateral steering without ever needing a physical hardware accelerometer hooked up to the Orange Pi!

---

## 🧪 8. HARDWARE-IN-THE-LOOP (HIL) BENCH TESTING
This Orange Pi 5 port is fully capable of being bench-tested using a mock steering column (like a RetroPilot EPS kit) and a monitor playing driving footage.

### How Screen-Spoofing Works with `NO_IMU=1`
Because this fork can run purely on **Visual Odometry** (`NO_IMU=1`), it does not require actual physical car movement (G-forces) to calculate trajectory. 
1. **Vision:** Point the IMX415 camera at a TV monitor playing dashboard highway footage. The NPU Vision Model will parse the lane lines, and Openpilot will calculate synthetic pitch, roll, and lateral speed purely by watching the pixels shift on the screen.
2. **CAN Simulation:** The physical Panda connected via USB to the Orange Pi must receive a spoofed vehicle speed (`msg.vEgo > 0`). Openpilot refuses to output steering torque if it mathematically believes the car is parked.
3. **NNLC Torque Output:** When a curve appears in the video on your TV, the Vision Model plans the path, the Neural Network Lateral Control (NNLC) calculates the required torque, and `controlsd` sends the physical CAN commands through the Panda. The bench steering wheel will physically turn to match the curve on the screen!

*Caveat: Ensure the IMX415 camera is positioned at a realistic distance from the TV. If the FOV/perspective is severely warped due to being too close to the screen, the `900.0` focal length math (`camera.py`) will miscalculate the curve sharpness, resulting in disproportionately weak or aggressive steering torque.*
